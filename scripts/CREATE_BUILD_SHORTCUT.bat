@echo off
REM Create Safe Build Shortcut on Desktop (OneDrive Compatible)
REM Created: 2025-10-07

set "scriptDir=%~dp0"
set "targetScript=%scriptDir%SAFE_BUILD.bat"
...
set "shortcutPath=%desktopPath%\SAFE_BUILD.lnk"

echo.
echo Creating Safe Build shortcut...
echo Target: %targetScript%
echo Desktop: %desktopPath%
echo.

REM Create temporary VBScript
set "vbsFile=%TEMP%\CreateSafeBuildShortcut.vbs"
...
echo oLink.Description = "Safe Flutter Build (Auto-verify + Auto-fix)" >> "%vbsFile%"
echo oLink.IconLocation = "shell32.dll,162" >> "%vbsFile%"
...
if exist "%shortcutPath%" (
    ...
    echo =====================================
    echo.
    echo Shortcut created: SAFE_BUILD
    echo Location: %desktopPath%
    echo.
    echo Double-click "SAFE_BUILD" on Desktop to build safely!
    echo.
) else (
    echo =====================================
